...
impl ChannelMarkers {
    ...
    /// literals; [`crate::zoo::ModelFamily::channel_markers`] maps
    ...
    /// marker literals; [`crate::zoo::ModelFamily::channel_markers`]
    ...
    /// [`crate::zoo::ModelFamily::channel_markers`] — this reader
            ...
            match self.state {
                ChannelState::Visible => {
                    if let Some(mlen) = match_any(&buf[i..], &self.markers.open) {
                        // Same deferral `push_emit` makes: a longer marker
                        // sharing this prefix may still complete on the next
                        // chunk. Committing early to the short form strands
                        // the difference — `<|channel>thought` vs
                        // `<|channel>thought\n` leaves a stray `\n` at the
                        // head of the reasoning block — and makes `push`
                        // disagree with `push_emit` on the same input.
                        if longer_pattern_could_extend(&buf[i..], mlen, &self.markers.open) {
                            self.pending = buf[i..].to_string();
                            return;
                        }
                ...
                ChannelState::InReasoning => {
                    if let Some(mlen) = match_any(&buf[i..], &self.markers.close) {
                        if longer_pattern_could_extend(&buf[i..], mlen, &self.markers.close) {
                            self.pending = buf[i..].to_string();
                            return;
                        }
...
fn match_any(s: &str, patterns: &[String]) -> Option<usize> {
    ...
    for p in patterns {
        // An empty marker matches at every position and consumes nothing.
        // Honouring it would flip the channel without advancing the scan
        // cursor, spinning the generation loop forever on the first byte.
        // `ChannelMarkers` is a plain struct with public fields, so an
        // empty marker is one field assignment away.
        if p.is_empty() {
            continue;
        }
...
mod tests {
    ...
    //
    // Everything this state machine consumes is generated by a model, and a
    // model can be steered by whoever writes the prompt. The contract: any
    // sequence of chunks terminates, never panics, and never loses or
    // duplicates a byte of the text it was given.

    /// Feed `chunks` through `push`, returning the split.
    fn via_push(markers: ChannelMarkers, chunks: &[&str]) -> ReplyParts {
    ...
    /// Feed `chunks` through `push_emit`, returning the split. Also asserts
    /// that the emissions and the internal buffers agree, since the
    /// streaming client sees the former and the stored reply the latter.
    fn via_push_emit(markers: ChannelMarkers, chunks: &[&str]) -> ReplyParts {
        let mut sm = ReplyStateMachine::new(markers);
        let mut streamed_content = String::new();
        let mut streamed_reasoning = String::new();
        for c in chunks {
            for e in sm.push_emit(c) {
                match e {
        ...
        let parts = sm.finish();
        assert_eq!(
            streamed_content, parts.content,
            "streamed content diverged from the stored reply"
        );
        assert_eq!(
            streamed_reasoning,
            parts.reasoning.clone().unwrap_or_default(),
            "streamed reasoning diverged from the stored reply"
        );
        parts
    }

    /// Every way of cutting `text` into `n` pieces on char boundaries is not
    /// enumerable, so take the sliding window of every single split point
    /// plus the whole string.
    fn all_two_way_splits(text: &str) -> Vec<Vec<&str>> {
        let mut out = vec![vec![text]];
        for i in 1..text.len() {
            if text.is_char_boundary(i) {
                out.push(vec![&text[..i], &text[i..]]);
            }
        }
        out
    }
    ...
    #[test]
    fn an_empty_marker_does_not_spin_the_scanner_forever() {
        // An empty marker matches at every offset and consumes nothing, so
        // an unguarded scanner flips channel without advancing and never
        // terminates. `ChannelMarkers` has public fields, so this is one
        // assignment away from any caller.
        let markers = ChannelMarkers {
            open: vec![String::new()],
            close: vec![String::new()],
        };
        let parts = via_push(markers.clone(), &["hello world"]);
        assert_eq!(parts.content, "hello world");
        assert_eq!(parts.reasoning, None);

        let streamed = via_push_emit(markers, &["hello world"]);
        assert_eq!(streamed.content, "hello world");
    }

    #[test]
    fn an_empty_marker_mixed_with_a_real_one_does_not_spin_the_scanner() {
        let markers = ChannelMarkers {
            open: vec![String::new(), "<think>".into()],
            close: vec!["</think>".into(), String::new()],
        };
        let parts = via_push(markers, &["a<think>b</think>c"]);
        assert_eq!(parts.content, "ac");
        assert_eq!(parts.reasoning.as_deref(), Some("b"));
    }

    #[test]
    fn an_empty_chunk_is_a_no_op() {
        let parts = via_push(gemma(), &["", "", "hello", "", ""]);
        assert_eq!(parts.content, "hello");
    }

    #[test]
    fn a_marker_delivered_one_byte_at_a_time_still_terminates_and_splits() {
        let text = "<think>reason</think>answer";
        let chunks: Vec<String> = text.chars().map(|c| c.to_string()).collect();
        let refs: Vec<&str> = chunks.iter().map(String::as_str).collect();
        let parts = via_push(ChannelMarkers::qwen3_deepseek(), &refs);
    ...
    #[test]
    fn push_and_push_emit_agree_on_a_marker_split_at_a_token_boundary() {
        // `<|channel>thought` is a plausible single token, so the `\n` of
        // `<|channel>thought\n` arrives in the *next* chunk. `push`
        // committed to the short marker there and left a stray `\n` at the
        // head of the reasoning block, while `push_emit` deferred and did
        // not — the same stream, split two different ways.
        let chunks = ["<|channel>thought", "\nreasoning\n<channel|>", "answer"];
        let batch = via_push(gemma(), &chunks);
        let streamed = via_push_emit(gemma(), &chunks);
        assert_eq!(batch, streamed);
        assert_eq!(batch.reasoning.as_deref(), Some("reasoning"));
        assert_eq!(batch.content, "answer");
    }

    #[test]
    fn push_and_push_emit_agree_across_every_single_split_of_a_full_turn() {
        // The chunk boundary is the model's to choose; the split must not be.
        for (markers, text) in [
            (
                gemma(),
                "<|channel>thought\nthinking hard\n<channel|>The answer.",
            ),
            (ChannelMarkers::qwen3_deepseek(), "<think>hmm</think>done"),
            (ChannelMarkers::none(), "plain text only"),
        ] {
            for chunks in all_two_way_splits(text) {
                let batch = via_push(markers.clone(), &chunks);
                let streamed = via_push_emit(markers.clone(), &chunks);
                assert_eq!(batch, streamed, "split {chunks:?} of {text:?} diverged");
            }
        }
    }

    #[test]
    fn the_split_of_a_full_turn_is_the_same_however_it_is_chunked() {
        let text = "<|channel>thought\nthinking hard\n<channel|>The answer.";
        let whole = via_push(gemma(), &[text]);
        for chunks in all_two_way_splits(text) {
            assert_eq!(
                via_push(gemma(), &chunks),
                whole,
                "chunking at {chunks:?} changed the split"
            );
        }
    }
    ...
    #[test]
    fn no_byte_of_unmarked_output_is_lost_or_duplicated() {
        // With no markers configured every byte must land in `content`
        // exactly once, whatever it looks like.
        for text in [
            "",
            "<",
            "<|",
            ...
            for chunks in all_two_way_splits(text) {
                let parts = via_push(ChannelMarkers::none(), &chunks);
                assert_eq!(parts.content, text, "text {text:?} split {chunks:?}");
                assert_eq!(parts.reasoning, None);
            }
        }
    }
    ...
    #[test]
    fn concatenating_the_two_channels_recovers_the_input_minus_its_markers() {
        // The only bytes the machine may drop are the markers themselves.
        let text = "before<think>middle</think>after";
        let parts = via_push(ChannelMarkers::qwen3_deepseek(), &[text]);
        let rejoined = format!(
            "before{}</think>{}",
            parts.reasoning.clone().unwrap_or_default(),
            parts.content.trim_start_matches("before")
        );
        assert_eq!(rejoined, "beforemiddle</think>after");
        assert_eq!(parts.content, "beforeafter");
    }

    // ── adversarial marker sequences ───────────────────────────────────

    #[test]
    fn a_close_marker_with_no_open_marker_is_ordinary_content() {
        // A prompt-injected `</think>` in the user's message can come back
        // verbatim in the reply. It must not open a reasoning channel or
        // swallow the visible answer.
        let parts = via_push(ChannelMarkers::qwen3_deepseek(), &["</think>the answer"]);
        assert_eq!(parts.content, "</think>the answer");
        assert_eq!(parts.reasoning, None);
    }

    #[test]
    fn an_unclosed_open_marker_keeps_everything_after_it_as_reasoning() {
        // Truncation mid-thought: the visible reply is empty rather than
        // leaking the chain of thought into `content`.
        let parts = via_push(gemma(), &["<|channel>thought\nsecret plan"]);
        assert_eq!(parts.content, "");
        assert_eq!(parts.reasoning.as_deref(), Some("secret plan"));
    }

    #[test]
    fn repeated_open_markers_do_not_stack_they_toggle() {
        // A model that emits `<think>` twice has one channel, not two. The
        // second is consumed as a marker while already inside reasoning?
        // No — inside reasoning only *close* markers match, so the second
        // `<think>` is reasoning text.
        let parts = via_push(
            ChannelMarkers::qwen3_deepseek(),
            &["<think>one<think>two</think>visible"],
        );
        assert_eq!(parts.reasoning.as_deref(), Some("one<think>two"));
        assert_eq!(parts.content, "visible");
    }

    #[test]
    fn alternating_markers_toggle_the_channel_every_time() {
        let parts = via_push(
            ChannelMarkers::qwen3_deepseek(),
            &["a<think>b</think>c<think>d</think>e"],
        );
        assert_eq!(parts.content, "ace");
        assert_eq!(parts.reasoning.as_deref(), Some("bd"));
    }

    #[test]
    fn a_marker_that_is_also_a_prefix_of_another_marker_takes_the_longest_match() {
        let markers = ChannelMarkers {
            open: vec!["<a>".into(), "<a><b>".into()],
            close: vec!["</a>".into()],
        };
        // The longest open marker wins, so `<b>` is not reasoning text.
        let parts = via_push(markers, &["<a><b>think</a>say"]);
        assert_eq!(parts.reasoning.as_deref(), Some("think"));
        assert_eq!(parts.content, "say");
    }

    #[test]
    fn an_open_marker_that_is_also_the_close_marker_still_toggles() {
        let markers = ChannelMarkers {
            open: vec!["|".into()],
            close: vec!["|".into()],
        };
        let parts = via_push(markers, &["a|b|c"]);
        assert_eq!(parts.content, "ac");
        assert_eq!(parts.reasoning.as_deref(), Some("b"));
    }

    #[test]
    fn a_reply_that_is_nothing_but_markers_yields_two_empty_channels() {
        let parts = via_push(ChannelMarkers::qwen3_deepseek(), &["<think></think>"]);
        assert_eq!(parts.content, "");
        assert_eq!(
            parts.reasoning, None,
            "an empty reasoning block must not be reported as present"
        );
    }
    ...
    #[test]
    fn a_flood_of_marker_prefixes_does_not_grow_the_held_back_buffer() {
        // `pending` holds bytes that might still complete a marker. If it
        // grew with the stream, a model emitting `<` forever would grow it
        // without bound and re-copy it on every push (quadratic).
        // It is bounded by the longest marker instead.
        let longest = ChannelMarkers::gemma4()
            .open
            .iter()
            .chain(ChannelMarkers::gemma4().close.iter())
            .map(String::len)
            .max()
            .unwrap();
        let mut sm = ReplyStateMachine::new(gemma());
        ...
        let parts = sm.finish();
        // Every byte is content, and none was held back forever.
        assert_eq!(parts.content.len(), 20_000 * 6);
        assert!(longest < 64, "marker length assumption changed");
    }

    #[test]
    fn a_long_run_of_special_token_shapes_counts_but_does_not_stall() {
        let mut sm = ReplyStateMachine::new(ChannelMarkers::none());
        let text = "<|a|>".repeat(5_000);
        sm.push(&text);
        let (_, _, specials) = sm.summary();
        assert!(specials > 0);
        assert_eq!(sm.finish().content, text);
    }

    #[test]
    fn the_leaked_special_counter_saturates_instead_of_wrapping() {
        // u32 counters over model output: the count is telemetry, and
        // telemetry must not wrap to zero on a pathological reply.
        let mut sm = ReplyStateMachine::new(ChannelMarkers::none());
        sm.push(&"<|".repeat(100_000));
        let (_, _, specials) = sm.summary();
        assert!(
            specials >= 100_000,
            "expected a large count, got {specials}"
        );
    }

    #[test]
    fn the_token_counters_saturate_instead_of_wrapping() {
        let mut sm = ReplyStateMachine::new(ChannelMarkers::none());
        for _ in 0..1000 {
            sm.push_emit("x");
        }
    ...
    #[test]
    fn a_multibyte_char_split_across_chunks_is_not_corrupted() {
        // Chunks arrive as `&str`, so each is valid UTF-8 on its own; the
        // risk is the scanner slicing one at a non-boundary.
        for text in [
            "héllo wörld 🌍",
            "日本語テキスト",
            "🇬🇧🇺🇸",
            ...
            for chunks in all_two_way_splits(text) {
                let parts = via_push(gemma(), &chunks);
                assert_eq!(parts.content, text, "{text:?} at {chunks:?}");
            }
        }
    }

    #[test]
    fn a_marker_wrapped_in_multibyte_text_is_still_found() {
        let parts = via_push(
            ChannelMarkers::qwen3_deepseek(),
            &["🌍<think>思考</think>🌎"],
        );
        assert_eq!(parts.content, "🌍🌎");
        assert_eq!(parts.reasoning.as_deref(), Some("思考"));
    }

    #[test]
    fn a_marker_containing_multibyte_text_is_matched_whole() {
        let markers = ChannelMarkers {
            open: vec!["【思考】".into()],
            close: vec!["【回答】".into()],
        };
        for chunks in all_two_way_splits("a【思考】b【回答】c") {
            let parts = via_push(markers.clone(), &chunks);
            assert_eq!(parts.content, "ac", "split {chunks:?}");
            assert_eq!(parts.reasoning.as_deref(), Some("b"));
        }
    }

    // ── property: arbitrary chunkings of arbitrary text ────────────────

    proptest::proptest! {
        #![proptest_config(proptest::prelude::ProptestConfig::with_cases(400))]

        /// Whatever a model emits, however it is chunked: the scan
        /// terminates, and `content + reasoning` is the input with only
        /// marker occurrences removed.
        #[test]
        fn no_chunking_of_any_output_loses_a_byte_that_is_not_a_marker(
            chunks in proptest::collection::vec(
                proptest::string::string_regex("(<|\\\\||>|think|/|channel|thought|\\n|[a-z ]){0,12}")
                    .unwrap(),
                0..12,
            ),
        ) {
            ...
            // With no markers at all, nothing may be dropped.
            let plain = via_push(ChannelMarkers::none(), &refs);
            proptest::prop_assert_eq!(&plain.content, &joined);
            proptest::prop_assert_eq!(plain.reasoning, None);

            // With markers, the two channels together may only be shorter
            // by whole markers, never longer, and never gain bytes.
            for markers in [gemma(), ChannelMarkers::qwen3_deepseek()] {
                let parts = via_push(markers, &refs);
                let kept = parts.content.len() + parts.reasoning.map_or(0, |r| r.len());
                proptest::prop_assert!(
                    kept <= joined.len(),
                    "scanner invented bytes: {} kept from {}", kept, joined.len()
                );
            }
        }

        /// `push` and `push_emit` are two spellings of one contract.
        #[test]
        fn push_and_push_emit_never_disagree(
            chunks in proptest::collection::vec(
                proptest::string::string_regex("(<\\\\|channel>thought|<channel\\\\|>|<think>|</think>|\\n|[ab])*")
                    .unwrap(),
                0..8,
            ),
        ) {
            let refs: Vec<&str> = chunks.iter().map(String::as_str).collect();
            for markers in [gemma(), ChannelMarkers::qwen3_deepseek(), ChannelMarkers::none()] {
                proptest::prop_assert_eq!(
                    via_push(markers.clone(), &refs),
                    via_push_emit(markers, &refs)
                );
            }
        }

        /// Arbitrary bytes-as-text with arbitrary markers: any answer is
        /// acceptable, non-termination and panics are not.
        #[test]
        fn arbitrary_markers_over_arbitrary_text_always_terminate(
            open in proptest::collection::vec(".{0,4}", 0..3),
            close in proptest::collection::vec(".{0,4}", 0..3),
            chunks in proptest::collection::vec(".{0,16}", 0..6),
        ) {
            let markers = ChannelMarkers { open, close };
            let refs: Vec<&str> = chunks.iter().map(String::as_str).collect();
            let _ = via_push(markers.clone(), &refs);
            let mut sm = ReplyStateMachine::new(markers);
            for c in &refs {
                let _ = sm.push_emit(c);
            }
            let _ = sm.flush_pending();
            let _ = sm.finish();
        }
    }
}
