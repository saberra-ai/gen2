...
pub struct ExecutionStats {
    ...
    /// Wall-clock milliseconds the reasoning channel streamed before the
    /// first visible content (unsloth-adoption 07). Measured by the
    /// transport-side channel splitter, so it reflects what the user
    /// actually watched. `None` when the turn produced no reasoning.
    #[serde(default)]
