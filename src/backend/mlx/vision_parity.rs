...
fn stage6_end_to_end_caption() {
    ...
    {
        use crate::backend::mlx::Engine;
        use crate::engine::{Capabilities, LoadRequest};
        let e = Engine::new();
    ...
    let (model, _cfg) = build_gemma4_model(&bundle).expect("build_gemma4_model");
    let tok = crate::backend::mlx::tokenizer::HfTokenizer::from_dir(&bundle).expect("tok");
...
fn stage7_real_path_caption() {
    use crate::Message;
    use crate::backend::mlx::Engine;
    use crate::engine::{Capabilities, LoadRequest, Settings};
    use crate::generation::{GenSpec, ThinkingMode, TokenEvent};
