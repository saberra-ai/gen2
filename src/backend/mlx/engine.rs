...
fn build_bundle_from_dir(model_dir: &Path) -> Result<ModelBundle, ExecError> {
    ...
    let meta = crate::backend::common::compute_hf_model_meta(
        &tokenizer,
        model_dir,
...
mod tests {
    ...
    use super::*;
    use crate::engine::{EmbedLoadRequest, ExecError, LoadRequest};
    ...
    fn multiturn_ten_turns() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let drain = |puller: &mut crate::backend::mlx::puller::TokenPuller| -> String {
    ...
    fn prefix_lru_holds_multiple_prefixes() -> Result<(), Box<dyn std::error::Error>> {
        use crate::Message;
        use crate::backend::mlx::puller::TokenPuller;
