...
use crate::backend::traits::{Backend, BackendSession, Embeddings, LocalBackend, Multimodal};
...
mod tests {
    ...
    fn kv_cache_persist_and_reload() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let snap = s_cache.save_cache(crate::kv::KvSaveSpec::ToPath(kv_path.clone()))?;
        ...
        let report = s2.load_cache(crate::kv::KvLoadSpec::Strict(kv_path))?;
